...
func (v *VScan) Init() {
	...
	if data, err := f.ReadFile("probes/nmap-service-probes"); err == nil {
		content = string(data)
	} else {
		panic(err)
	}
...
func (v *VScan) Check(protocol, host string, port int) (string, string, string) {
	...
	// 	target.Port,
	// 	result.Name,
	// 	result.Extras.VendorProduct,
	// 	result.Extras.Version,
	// )
	if err != nil {
		return "", "", ""
	}
	return result.Name, result.Extras.VendorProduct, result.Extras.Version
}
