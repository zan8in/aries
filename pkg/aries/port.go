...
const (
	...
	DatabaseTop = "523,1433,1434,1521,1583,2100,2049,2638,3050,3306,3351,5000,5432,5433,5601,5984,6082,6379,7474,8080,8088,8089,8098,8471,9000,9160,9200,9300,9471,11211,11211,15672,19888,27017,27019,27080,28017,50000,50070,50090"
	HotelTop    = "21,22,23,25,53,80,110,111,111,135,137,139,161,389,443,445,515,548,873,902,1080,1433,1434,1900,2181,2375,2379,3128,3306,3389,5060,5222,5351,5353,5555,5672,5683,5900,6379,7001,8080,9000,9100,9200,11211,11211,37810"
	ICSTop      = "21,22,23,80,102,161,443,502,1089-1091,2222,3671,4000,4000,4840,5050,9600,20000,20000,47808,34962-34964,34980,44818,44818,55000-55003"
	IoTTop      = "21,22,23,25,80,81,82,83,84,88,137,143,443,445,554,631,1080,1883,1900,2000,2323,3671,3702,4433,4443,4567,5222,5683,7474,7547,8000,8023,8080,8081,8443,8088,8600,8883,8888,9000,9090,9999,10000,30718,37020,37777,37810,49152"
...
func ParsePorts(options *Options) ([]*port.Port, error) {
	...
	if options.TopPorts != "" {
		...
		case "mini": // If the user has specfied 1000, use them
			ports, err := parsePortsList(MiniTop)
			if err != nil {
				return nil, fmt.Errorf("could not read ports: %s", err)
			}
			topPortsCLIMap, err = excludePorts(options, ports)
		...
		case "database": // If the user has specfied 1000, use them
			ports, err := parsePortsList(DatabaseTop)
			if err != nil {
				return nil, fmt.Errorf("could not read ports: %s", err)
			}
			topPortsCLIMap, err = excludePorts(options, ports)
		...
		case "hotel": // If the user has specfied 1000, use them
			ports, err := parsePortsList(HotelTop)
			if err != nil {
				return nil, fmt.Errorf("could not read ports: %s", err)
			}
			topPortsCLIMap, err = excludePorts(options, ports)
		...
		case "iot": // If the user has specfied 1000, use them
			ports, err := parsePortsList(IoTTop)
			if err != nil {
				return nil, fmt.Errorf("could not read ports: %s", err)
			}
			topPortsCLIMap, err = excludePorts(options, ports)
		...
		case "ics": // If the user has specfied 1000, use them
			ports, err := parsePortsList(ICSTop)
			if err != nil {
				return nil, fmt.Errorf("could not read ports: %s", err)
			}
			topPortsCLIMap, err = excludePorts(options, ports)
